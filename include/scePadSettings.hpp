...
struct s_scePadSettings {
	...
	std::array<std::array<int, 11>, TRIGGER_COUNT> uiParameters = {
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(s_scePadSettings,
												udpConfig,
												led,
												audioToLed,
												brightness,
...
const std::unordered_map<std::string, TriggerHandler> sonyTriggerHandlers = {
	{TriggerStringSony::OFF,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>&) {
		 uint8_t index = s.uiSelectedTrigger == L2 ? SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2
	...
	{TriggerStringSony::FEEDBACK,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 if (p.size() < 2) return;
	...
	{TriggerStringSony::MULTIPLE_POSITION_VIBRATION,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 ...
		 for (int i = 1; i < 11; ++i)
			 s.stockTriggerParam.command[triggerIndex].commandData.multiplePositionVibrationParam.amplitude[i - 1] =
				 p[i];
	 }}};

const std::unordered_map<std::string, TriggerHandler> dsxTriggerHandlers = {
	{TriggerStringDSX::Normal,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerNormal(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::GameCube,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerGamecube(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::VerySoft,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerVerySoft(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Soft,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerSoft(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Medium,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerMedium(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Hard,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerHard(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::VeryHard,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerVeryHard(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Hardest,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerHardest(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::VibrateTrigger,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerVibrateTrigger(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::VibrateTriggerPulse,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerVibrateTriggerPulse(triggerIndex == L2 ? s.leftCustomTrigger.data()
															 : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Choppy,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerChoppy(triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::CustomTriggerValue,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerCustomTriggerValue(p,
										 triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Resistance,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerResistance(p, triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Bow,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerBow(p, triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Galloping,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerGalloping(p, triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::SemiAutomaticGun,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerSemiAutomaticGun(p,
									   triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::AutomaticGun,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerAutomaticGun(p, triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::Machine,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerMachine(p, triggerIndex == L2 ? s.leftCustomTrigger.data() : s.rightCustomTrigger.data());
	 }},
	{TriggerStringDSX::VIBRATE_TRIGGER_10Hz,
	 [](s_scePadSettings& s, int& triggerIndex, const std::vector<uint8_t>& p) {
		 CustomTriggerVIBRATE_TRIGGER_10Hz(p,
										   triggerIndex == L2 ? s.leftCustomTrigger.data()
															  : s.rightCustomTrigger.data());
	 }},
};
