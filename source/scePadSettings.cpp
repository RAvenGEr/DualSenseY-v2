...
bool GetDefaultConfigFromMac(const std::string& mac, s_scePadSettings* s) {
	...
	std::filesystem::path filePath =
		std::filesystem::path(sago::getDocumentsFolder() + "/DSY/DefaultConfigs/" + cleanMac);
...
void applySettings(uint32_t index, s_scePadSettings settings, AudioPassthrough& audio) {
	...
	if (settings.useLightbarFromEmulatedController &&
		(settings.emulatedController == (int)EmulatedController::DUALSHOCK4 || settings.usingPeerController)) {
		...
		s_SceLightBar lightbar = {std::min(audioPeakUint8, rainbowLightbar.r),
								  std::min(audioPeakUint8, rainbowLightbar.g),
								  std::min(audioPeakUint8, rainbowLightbar.b)};
		scePadSetLightBar(g_ScePad[index], &lightbar);
	}
		...
		s_SceLightBar lightbar = {(uint8_t)scaleFloatToInt(settings.led[0], 1.0f),
								  (uint8_t)scaleFloatToInt(settings.led[1], 1.0f),
								  (uint8_t)scaleFloatToInt(settings.led[2], 1.0f)};
		scePadSetLightBar(g_ScePad[index], &lightbar);
	}
	...
	int l2Value = settings.rumbleToAt_swapTriggers ? settings.rumbleFromEmulatedController.smallMotor
												   : settings.rumbleFromEmulatedController.largeMotor;
	int r2Value = settings.rumbleToAt_swapTriggers ? settings.rumbleFromEmulatedController.largeMotor
												   : settings.rumbleFromEmulatedController.smallMotor;

	if (settings.triggersAsButtons &&
		(settings.usingPeerController || settings.emulatedController != (int)EmulatedController::NONE)) {
		...
		settings.stockTriggerParam.triggerMask |=
			settings.isLeftUsingDsxTrigger ? 0 : SCE_PAD_TRIGGER_EFFECT_TRIGGER_MASK_L2;
		settings.stockTriggerParam.triggerMask |=
			settings.isRightUsingDsxTrigger ? 0 : SCE_PAD_TRIGGER_EFFECT_TRIGGER_MASK_R2;
		...
		scePadSetTriggerEffectCustom(g_ScePad[index],
									 settings.leftCustomTrigger.data(),
									 settings.rightCustomTrigger.data(),
									 triggerBitmask);
	}

	if (settings.useRumbleFromEmulatedController || settings.udpConfig) {
		if (settings.rumbleFromEmulatedController.largeMotor > 0 ||
			settings.rumbleFromEmulatedController.smallMotor > 0) {
