...
static GUID StringToGuid(const std::string& guidStr) {
	...
	int matched = std::sscanf(guidStr.c_str(),
...
static ma_result
FindDeviceByContainerIdWindows(ma_context* pContext, const GUID& targetContainerId, ma_device_id* pDeviceId) {
...
std::optional<std::string> ExtractUsbPort(const std::string& path) {
	...
	if (std::regex_search(path, m, re) && m.size() > 1) {
		...
		auto pos = token.find(':');
		if (pos != std::string::npos) token.resize(pos);
		return token;
	}
	return std::nullopt;
}
...
bool SameDevice(const std::string& a, const std::string& b) {
	if (SamePhysicalDeviceByUsbPort(a, b)) return true;
...
ma_result
FindDeviceByContainerIdLinux(ma_context* pContext, const std::string& targetContainerId, ma_device_id* pDeviceId) {
	ma_device_info* pPlaybackInfos;
	ma_uint32 playbackCount;
...
AudioPassthrough::~AudioPassthrough() {
	...
	for (uint32_t i = 0; i < 4; i++) {
		if (isMaDeviceWorking(&m_Controller[i])) ma_device_uninit(&m_Controller[i]);
	}
...
void AudioPassthrough::HapticTimerThread() {
	...
	while (g_running) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (result != SCE_OK || busType != SCE_PAD_BUSTYPE_BT) continue;

			hid_device* device = (hid_device*)scePadGetHidApiHandle(g_ScePad[i]);
			if (device == nullptr || m_Active[i] == false) continue;
...
void AudioPassthrough::Validate() {
	...
	if (m_TimeSinceLastRetry < std::chrono::seconds(5)) return;
...
void PlaybackDualsenseDataCallback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
	...
	userData->m_AudioBuffer[index].erase(userData->m_AudioBuffer[index].begin(),
										 userData->m_AudioBuffer[index].begin() + framesToWrite * 2);
}
...
bool AudioPassthrough::StartByUserId(int userId) {
	...
	playbackConfig.dataCallback = controllerType == s_SceControllerType::DUALSENSE ? PlaybackDualsenseDataCallback
																				   : PlaybackDualshock4DataCallback;
