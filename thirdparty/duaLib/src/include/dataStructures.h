...
namespace dualsenseData {
	...
	struct USBGetStateData {  // 63
		...
		/*41.0*/ uint8_t TriggerRightStopLocation
			: 4;  // trigger stop can be a range from 0 to 9 (F/9.0 for Apple interface)
	...
	struct SetStateData {  // 47
		...
		/*37.3*/ uint8_t BeamformingEnable
			: 1;  // Probably for MIC given there's 2, might be more bits, can't find what it does
		...
		bool operator==(const SetStateData& other) const {
			return std::memcmp(RightTriggerFFB, other.RightTriggerFFB, 11) == 0 &&
				   std::memcmp(LeftTriggerFFB, other.LeftTriggerFFB, 11) == 0 && LedRed == other.LedRed &&
				   LedGreen == other.LedGreen && LedBlue == other.LedBlue && PlayerLight1 == other.PlayerLight1 &&
				   PlayerLight2 == other.PlayerLight2 && PlayerLight3 == other.PlayerLight3 &&
				   PlayerLight4 == other.PlayerLight4 && PlayerLight5 == other.PlayerLight5 &&
				   MuteLightMode == other.MuteLightMode && RumbleEmulationLeft == other.RumbleEmulationLeft &&
				   RumbleEmulationRight == other.RumbleEmulationRight && VolumeSpeaker == other.VolumeSpeaker &&
				   VolumeMic == other.VolumeMic && VolumeHeadphones == other.VolumeHeadphones &&
				   lightBrightness == other.lightBrightness && OutputPathSelect == other.OutputPathSelect;
		}

		bool operator!=(const SetStateData& other) const { return !(*this == other); }
	};
...
#pragma pack(push, 1)
	...
	struct BTSetStateData {
		...
		bool operator==(const BTSetStateData& other) const {
			return LedRed == other.LedRed && LedGreen == other.LedGreen && LedBlue == other.LedBlue &&
				   RumbleLeft == other.RumbleLeft && RumbleRight == other.RumbleRight &&
				   VolumeSpeaker == other.VolumeSpeaker && VolumeMic == other.VolumeMic &&
				   VolumeLeft == other.VolumeLeft && VolumeRight == other.VolumeRight;
		}

		bool operator!=(const BTSetStateData& other) const { return !(*this == other); }
	};

	template <int N>
	struct BTSetStateDataAndAudio {
