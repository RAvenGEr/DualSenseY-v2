...
typedef struct ScePadTriggerEffectMultiplePositionFeedbackParam {
	uint8_t
		strength[SCE_PAD_TRIGGER_EFFECT_CONTROL_POINT_NUM]; /*E strength that the motor arm pushes back target trigger
															 * at position(0~8 (0: Same as Off mode)). strength[0] means
															 * strength of motor arm at position0. strength[1] means
...
typedef struct ScePadTriggerEffectMultiplePositionVibrationParam {
	...
	uint8_t amplitude[SCE_PAD_TRIGGER_EFFECT_CONTROL_POINT_NUM]; /*E vibration amplitude at position(0~8 (0: Same as Off
																  * mode)). amplitude[0] means amplitude of vibration at
																  * position0. amplitude[1] means amplitude of vibration
																  * at position1.
																  *  ...
																  * */
	uint8_t padding[37];
...
typedef struct ScePadTriggerEffectParam {
	...
	ScePadTriggerEffectCommand
		command[SCE_PAD_TRIGGER_EFFECT_TRIGGER_NUM]; /*E command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2] is for L2
													  * trigger setting and
													  * param[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_R2] is for R2
													  * trgger setting.
													  * */
...
// Declare C++ linkage (to return std::string)
