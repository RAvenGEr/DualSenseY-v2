 ...
 *
 * @param[in]  rd_item  Enumeration identifying type (Main, Global, Local) and function (e.g Usage or Report Count) of
 * the item.
...
static int rd_write_short_item(rd_items rd_item, LONG64 data, struct rd_buffer* rpt_desc) {
	...
	else if ((rd_item == rd_global_logical_minimum) || (rd_item == rd_global_logical_maximum) ||
			 (rd_item == rd_global_physical_minimum) || (rd_item == rd_global_physical_maximum)) {
...
static struct rd_main_item_node* rd_append_main_item_node(int first_bit,
														  int last_bit,
														  rd_node_type type_of_node,
														  int caps_index,
														  int collection_index,
														  rd_main_items main_item_type,
...
static struct rd_main_item_node* rd_insert_main_item_node(int first_bit,
														  int last_bit,
														  rd_node_type type_of_node,
														  int caps_index,
														  int collection_index,
	...
	(*list)->next = NULL;
	rd_append_main_item_node(first_bit,
							 last_bit,
							 type_of_node,
							 caps_index,
							 collection_index,
...
static struct rd_main_item_node* rd_search_main_item_list_for_bit_position(int search_bit,
																		   rd_main_items main_item_type,
																		   unsigned char report_id,
																		   struct rd_main_item_node** list) {
	// Determine first INPUT/OUTPUT/FEATURE main item, where the last bit position is equal or greater than the search
	// bit position

	while (((*list)->next->MainItemType != rd_collection) && ((*list)->next->MainItemType != rd_collection_end) &&
		   !(((*list)->next->LastBit >= search_bit) && ((*list)->next->ReportID == report_id) &&
...
int hid_winapi_descriptor_reconstruct_pp_data(void* preparsed_data, unsigned char* buf, size_t buf_size) {
	...
	struct rd_buffer rpt_desc = {.buf = buf, .buf_size = buf_size, .byte_idx = 0};

	// Set pointer to the first node of link_collection_nodes
	phid_pp_link_collection_node link_collection_nodes =
		(phid_pp_link_collection_node)(((unsigned char*)&pp_data->caps[0]) + pp_data->FirstByteOfLinkCollectionArray);

	// ****************************************************************************************************************************
	// Create lookup tables for the bit range of each report per collection (position of first bit and last bit in each
	// collection) coll_bit_range[COLLECTION_INDEX][REPORT_ID][INPUT/OUTPUT/FEATURE]
	// ****************************************************************************************************************************
	...
	for (USHORT collection_node_idx = 0; collection_node_idx < pp_data->NumberLinkCollectionNodes;
		 collection_node_idx++) {
		coll_bit_range[collection_node_idx] =
			(rd_bit_range***)malloc(256 * sizeof(*coll_bit_range[0]));  // 256 possible report IDs (incl. 0x00)
		for (int reportid_idx = 0; reportid_idx < 256; reportid_idx++) {
			coll_bit_range[collection_node_idx][reportid_idx] =
				(rd_bit_range**)malloc(NUM_OF_HIDP_REPORT_TYPES * sizeof(*coll_bit_range[0][0]));
	...
	for (int rt_idx = 0; rt_idx < NUM_OF_HIDP_REPORT_TYPES; rt_idx++) {
		for (USHORT caps_idx = pp_data->caps_info[rt_idx].FirstCap; caps_idx < pp_data->caps_info[rt_idx].LastCap;
			 caps_idx++) {
			int first_bit, last_bit;
			first_bit = (pp_data->caps[caps_idx].BytePosition - 1) * 8 + pp_data->caps[caps_idx].BitPosition;
			last_bit = first_bit + pp_data->caps[caps_idx].ReportSize * pp_data->caps[caps_idx].ReportCount - 1;
			if (coll_bit_range[pp_data->caps[caps_idx].LinkCollection][pp_data->caps[caps_idx].ReportID][rt_idx]
						->FirstBit == -1 ||
				coll_bit_range[pp_data->caps[caps_idx].LinkCollection][pp_data->caps[caps_idx].ReportID][rt_idx]
						->FirstBit > first_bit) {
				coll_bit_range[pp_data->caps[caps_idx].LinkCollection][pp_data->caps[caps_idx].ReportID][rt_idx]
					->FirstBit = first_bit;
			}
	...
	int* coll_number_of_direct_childs =
		(int*)malloc(pp_data->NumberLinkCollectionNodes * sizeof(coll_number_of_direct_childs[0]));
	for (USHORT collection_node_idx = 0; collection_node_idx < pp_data->NumberLinkCollectionNodes;
		 collection_node_idx++) {
					...
					for (int reportid_idx = 0; reportid_idx < 256; reportid_idx++) {
						for (int rt_idx = 0; rt_idx < NUM_OF_HIDP_REPORT_TYPES; rt_idx++) {
							...
							if ((coll_bit_range[child_idx][reportid_idx][rt_idx]->FirstBit != -1) &&
								(coll_bit_range[collection_node_idx][reportid_idx][rt_idx]->FirstBit >
								 coll_bit_range[child_idx][reportid_idx][rt_idx]->FirstBit)) {
								coll_bit_range[collection_node_idx][reportid_idx][rt_idx]->FirstBit =
									coll_bit_range[child_idx][reportid_idx][rt_idx]->FirstBit;
							}
							if (coll_bit_range[collection_node_idx][reportid_idx][rt_idx]->LastBit <
								coll_bit_range[child_idx][reportid_idx][rt_idx]->LastBit) {
								coll_bit_range[collection_node_idx][reportid_idx][rt_idx]->LastBit =
									coll_bit_range[child_idx][reportid_idx][rt_idx]->LastBit;
							}
		...
		while (actual_coll_level >= 0) {
			if ((coll_number_of_direct_childs[collection_node_idx] != 0) &&
				...
				coll_child_order[collection_node_idx] =
					(USHORT*)malloc((coll_number_of_direct_childs[collection_node_idx]) * sizeof(*coll_child_order[0]));
				...
				if (coll_number_of_direct_childs[collection_node_idx] > 1) {
					...
					for (int rt_idx = 0; rt_idx < NUM_OF_HIDP_REPORT_TYPES; rt_idx++) {
						for (int reportid_idx = 0; reportid_idx < 256; reportid_idx++) {
							for (int child_idx = 1; child_idx < coll_number_of_direct_childs[collection_node_idx];
								 child_idx++) {
								// since the coll_bit_range array is not sorted, we need to reference the collection
								// index in our sorted coll_child_order array, and look up the corresponding bit ranges
								// for comparing values to sort
								...
								if ((coll_bit_range[prev_coll_idx][reportid_idx][rt_idx]->FirstBit != -1) &&
									...
									(coll_bit_range[prev_coll_idx][reportid_idx][rt_idx]->FirstBit >
									 coll_bit_range[cur_coll_idx][reportid_idx][rt_idx]->FirstBit)) {
									...
									coll_child_order[collection_node_idx][child_idx - 1] =
										coll_child_order[collection_node_idx][child_idx];
	...
	struct rd_main_item_node** coll_begin_lookup =
		(struct rd_main_item_node**)malloc(pp_data->NumberLinkCollectionNodes * sizeof(*coll_begin_lookup));
	struct rd_main_item_node** coll_end_lookup =
		(struct rd_main_item_node**)malloc(pp_data->NumberLinkCollectionNodes * sizeof(*coll_end_lookup));
	{
		int* coll_last_written_child =
			(int*)malloc(pp_data->NumberLinkCollectionNodes * sizeof(coll_last_written_child[0]));
		for (USHORT collection_node_idx = 0; collection_node_idx < pp_data->NumberLinkCollectionNodes;
			 collection_node_idx++) {
		...
		coll_begin_lookup[0] = rd_append_main_item_node(0,
														0,
		...
		while (actual_coll_level >= 0) {
			if ((coll_number_of_direct_childs[collection_node_idx] != 0) &&
				...
				if (link_collection_nodes[collection_node_idx].IsAlias && (firstDelimiterNode == NULL)) {
					// Alliased Collection (First node in link_collection_nodes -> Last entry in report descriptor
					// output)
					firstDelimiterNode = main_item_list;
					coll_begin_lookup[collection_node_idx] = rd_append_main_item_node(0,
																					  0,
																					  rd_item_node_collection,
																					  0,
																					  collection_node_idx,
					...
					actual_coll_level++;
					 ...
					 (coll_last_written_child[collection_node_idx] !=
					  coll_child_order[collection_node_idx][coll_number_of_direct_childs[collection_node_idx] - 1])) {
				// Collection has child collections, and this is not the first child

				int nextChild = 1;
				while (coll_last_written_child[collection_node_idx] !=
					   coll_child_order[collection_node_idx][nextChild - 1]) {
					nextChild++;
				}
				...
				if (link_collection_nodes[collection_node_idx].IsAlias && (firstDelimiterNode == NULL)) {
					...
					coll_begin_lookup[collection_node_idx] = rd_insert_main_item_node(0,
																					  0,
																					  rd_item_node_collection,
																					  0,
																					  collection_node_idx,
				...
				actual_coll_level--;
				coll_end_lookup[collection_node_idx] = rd_append_main_item_node(0,
																				0,
																				rd_item_node_collection,
																				0,
																				collection_node_idx,
	...
	for (int rt_idx = 0; rt_idx < NUM_OF_HIDP_REPORT_TYPES; rt_idx++) {
		...
		for (USHORT caps_idx = pp_data->caps_info[rt_idx].FirstCap; caps_idx < pp_data->caps_info[rt_idx].LastCap;
			...
			for (int child_idx = 0; child_idx < coll_number_of_direct_childs[pp_data->caps[caps_idx].LinkCollection];
				 child_idx++) {
				// Determine in which section before/between/after child collection the item should be inserted
				if (first_bit < coll_bit_range[coll_child_order[pp_data->caps[caps_idx].LinkCollection][child_idx]]
											  [pp_data->caps[caps_idx].ReportID][rt_idx]
												  ->FirstBit) {
					// Note, that the default value for undefined coll_bit_range is -1, which can't be greater than the
					// bit position
					break;
				}
			...
			list_node = rd_search_main_item_list_for_bit_position(first_bit,
																  (rd_main_items)rt_idx,
			...
			if (pp_data->caps[caps_idx].IsAlias && (firstDelimiterNode == NULL)) {
				...
				rd_insert_main_item_node(first_bit,
										 last_bit,
										 rd_item_node_cap,
										 caps_idx,
										 pp_data->caps[caps_idx].LinkCollection,
										 rd_delimiter_usage,
										 pp_data->caps[caps_idx].ReportID,
										 &list_node);
				rd_insert_main_item_node(first_bit,
	...
	// ***********************************************************
	{
		...
		while (list->next != NULL) {
			if ((list->MainItemType >= rd_input) && (list->MainItemType <= rd_feature)) {
				...
				if (list->FirstBit != -1) {
					if ((last_bit_position[list->MainItemType][list->ReportID] + 1 != list->FirstBit) &&
						...
						(last_report_item_lookup[list->MainItemType][list->ReportID]->FirstBit !=
						 list->FirstBit)  // Happens in case of IsMultipleItemsForArray for multiple dedicated usages
										  // for a multi-button array
					) {
						struct rd_main_item_node* list_node = rd_search_main_item_list_for_bit_position(
							last_bit_position[list->MainItemType][list->ReportID],
							list->MainItemType,
							list->ReportID,
							&last_report_item_lookup[list->MainItemType][list->ReportID]);
						rd_insert_main_item_node(last_bit_position[list->MainItemType][list->ReportID] + 1,
												 list->FirstBit - 1,
			...
			for (int reportid_idx = 0; reportid_idx < 256; reportid_idx++) {
				if (last_bit_position[rt_idx][reportid_idx] != -1) {
					...
					if (padding < 8) {
						...
						rd_insert_main_item_node(last_bit_position[rt_idx][reportid_idx] + 1,
												 last_bit_position[rt_idx][reportid_idx] + padding,
												 rd_item_node_padding,
												 -1,
												 0,
												 (rd_main_items)rt_idx,
												 (unsigned char)reportid_idx,
												 &last_report_item_lookup[rt_idx][reportid_idx]);
						if (last_report_item_lookup[rt_idx][reportid_idx] == node_before_top_level_coll_end) {
							// If this padding item is at the end of the top level collection, update
							// node_before_top_level_coll_end
		...
		// Add full byte padding at the end of the report descriptor (only reconstructable, for devices without Report
		// IDs)
		for (int rt_idx = 0; rt_idx < NUM_OF_HIDP_REPORT_TYPES; rt_idx++) {
			if (!devicehasReportIDs && pp_data->caps_info[rt_idx].NumberOfCaps > 0 &&
				pp_data->caps_info[rt_idx].ReportByteLength > 0) {
				// ReportID 0 means this device uses not Report IDs
				// => Maximum one report per type possible, so we can take the size from the buffer size for the report
				// type
				const unsigned char reportId = 0;
				// ReportByteLength is the report length in bytes plus the one byte for the optional ReportID
				int padding =
					(pp_data->caps_info[rt_idx].ReportByteLength - 1) * 8 - (last_bit_position[rt_idx][reportId] + 1);

				if (padding > 0) {
					// Insert padding item after item referenced in last_report_item_lookup
					rd_insert_main_item_node(last_bit_position[rt_idx][reportId] + 1,
											 last_bit_position[rt_idx][reportId] + padding,
											 rd_item_node_padding,
											 -1,
											 0,
											 (rd_main_items)rt_idx,
	...
	LONG last_physical_min = 0;  // If both, Physical Minimum and Physical Maximum are 0, the logical limits should be
								 // taken as physical limits according USB HID spec 1.11 chapter 6.2.2.7
	LONG last_physical_max = 0;
	ULONG last_unit_exponent =
		0;  // If Unit Exponent is Undefined it should be considered as 0 according USB HID spec 1.11 chapter 6.2.2.7
	ULONG last_unit = 0;  // If the first nibble is 7, or second nibble of Unit is 0, the unit is None according USB HID
						  // spec 1.11 chapter 6.2.2.7
	BOOLEAN inhibit_write_of_usage =
		FALSE;  // Needed in case of delimited usage print, before the normal collection or cap
	int report_count = 0;
	while (main_item_list != NULL) {
		...
		if (main_item_list->MainItemType == rd_collection) {
			...
			if ((main_item_list->LastBit - main_item_list->FirstBit + 1) % 8 == 0) {
				...
				rd_write_short_item(rd_global_report_count,
									(main_item_list->LastBit - main_item_list->FirstBit + 1) / 8,
									&rpt_desc);
			}
			else {
				// Padding in bits
				...
				rd_write_short_item(rd_global_report_size,
									(main_item_list->LastBit - main_item_list->FirstBit + 1),
									&rpt_desc);
			...
			if (rt_idx == HidP_Input) {
				...
				// matter in case of const
				rd_write_short_item(rd_main_input, 0x03, &rpt_desc);  // Const / Abs
			}
			else if (rt_idx == HidP_Output) {
				// Write "Output" main item - We know it's Constant - We can only guess the other bits, but they don't
				// matter in case of const
				rd_write_short_item(rd_main_output, 0x03, &rpt_desc);  // Const / Abs
			}
			else if (rt_idx == HidP_Feature) {
				// Write "Feature" main item - We know it's Constant - We can only guess the other bits, but they don't
				// matter in case of const
			...
			if (pp_data->caps[caps_idx].IsRange) {
				report_count +=
					(pp_data->caps[caps_idx].Range.DataIndexMax - pp_data->caps[caps_idx].Range.DataIndexMin);
			}
			...
			if (pp_data->caps[caps_idx].IsDesignatorRange) {
				...
				rd_write_short_item(rd_local_designator_minimum,
									pp_data->caps[caps_idx].Range.DesignatorMin,
									&rpt_desc);
				rd_write_short_item(rd_local_designator_maximum,
				...
				rd_write_short_item(rd_local_designator_index,
			...
			if (pp_data->caps[caps_idx].IsStringRange) {
				...
				// String Index 0 is a special entry of the USB string descriptor, that contains a list of supported
				// languages, therefore Designator Index 0 can never be a useful reference for a control and we can
				// inhibit it. Write single "String Index"
				rd_write_short_item(rd_local_string, pp_data->caps[caps_idx].NotRange.StringIndex, &rpt_desc);
			}

			if ((main_item_list->next != NULL) && ((int)main_item_list->next->MainItemType == rt_idx) &&
				...
				if (main_item_list->next->FirstBit != main_item_list->FirstBit) {
					// In case of IsMultipleItemsForArray for multiple dedicated usages for a multi-button array, the
					// report count should be incremented

					// Skip global items until any of them changes, than use ReportCount item to write the count of
					// identical report fields
					report_count++;
				}
			}
			else {
				if ((pp_data->caps[caps_idx].Button.LogicalMin == 0) &&
					...
					rd_write_short_item(rd_global_logical_minimum,
										pp_data->caps[caps_idx].Button.LogicalMin,
										&rpt_desc);
					rd_write_short_item(rd_global_logical_maximum,
				...
				if (!pp_data->caps[caps_idx].IsRange) {
					...
					// In case of multiple usages with the same items, only "Usage" is written per cap, and "Report
			...
			if ((pp_data->caps[caps_idx].BitField & 0x02) != 0x02) {
				...
				pp_data->caps[caps_idx].ReportCount =
					pp_data->caps[caps_idx].Range.DataIndexMax - pp_data->caps[caps_idx].Range.DataIndexMin + 1;
			}
			...
			if ((main_item_list->next != NULL) && ((int)main_item_list->next->MainItemType == rt_idx) &&
				...
				(!pp_data->caps[main_item_list->next->CapsIndex].IsRange) &&  // Next node in list is no array
				(pp_data->caps[main_item_list->next->CapsIndex].UsagePage == pp_data->caps[caps_idx].UsagePage) &&
				(pp_data->caps[main_item_list->next->CapsIndex].NotButton.LogicalMin ==
				 pp_data->caps[caps_idx].NotButton.LogicalMin) &&
				(pp_data->caps[main_item_list->next->CapsIndex].NotButton.LogicalMax ==
				 pp_data->caps[caps_idx].NotButton.LogicalMax) &&
				(pp_data->caps[main_item_list->next->CapsIndex].NotButton.PhysicalMin ==
				 pp_data->caps[caps_idx].NotButton.PhysicalMin) &&
				(pp_data->caps[main_item_list->next->CapsIndex].NotButton.PhysicalMax ==
				...
				if ((last_physical_min != pp_data->caps[caps_idx].NotButton.PhysicalMin) ||
					...
					rd_write_short_item(rd_global_physical_minimum,
										pp_data->caps[caps_idx].NotButton.PhysicalMin,
										&rpt_desc);
					last_physical_min = pp_data->caps[caps_idx].NotButton.PhysicalMin;
					rd_write_short_item(rd_global_physical_maximum,
										pp_data->caps[caps_idx].NotButton.PhysicalMax,
										&rpt_desc);
					last_physical_max = pp_data->caps[caps_idx].NotButton.PhysicalMax;
				}
