...
static hidp_preparsed_data* alloc_preparsed_data_from_file(char* filename) {
	...
	while (fgets(line, sizeof(line), file) != NULL) {
		...
		if (FirstByteOfLinkCollectionArray != 0 && NumberLinkCollectionNodes != 0) {
			size_t size_of_preparsed_data = offsetof(hidp_preparsed_data, caps) + FirstByteOfLinkCollectionArray +
											(NumberLinkCollectionNodes * sizeof(hid_pp_link_collection_node));
		...
		if (sscanf(
				line,
				"pp_data->MagicKey                             = 0x%02hhX%02hhX%02hhX%02hhX%02hhX%02hhX%02hhX%02hhX\n",
				&pp_data->MagicKey[0],
				&pp_data->MagicKey[1],
				&pp_data->MagicKey[2],
				&pp_data->MagicKey[3],
		...
		if (sscanf(line, "pp_data->caps_info[%u]", &rt_idx) == 1) {
			...
			if (rt_idx >= caps_info_count) {
				fprintf(
					stderr,
					"Broken pp_data file, pp_data->caps_info[<idx>] can have at most %zu elements, accessing %ud, (%s)",
					caps_info_count,
					rt_idx,
					line);
				continue;
		...
		if (sscanf(line,
				   "pp_data->FirstByteOfLinkCollectionArray       = 0x%04hX\n",
		...
		if (sscanf(line, "pp_data->cap[%u]", &caps_idx) == 1) {
			if (pp_data->FirstByteOfLinkCollectionArray == 0) {
				fprintf(stderr,
						"Error reading pp_data file (%s): FirstByteOfLinkCollectionArray is 0 or not reported yet\n",
						line);
				continue;
			}
			if ((caps_idx + 1) * sizeof(hid_pp_cap) > pp_data->FirstByteOfLinkCollectionArray) {
				fprintf(stderr,
						"Error reading pp_data file (%s): the caps index (%u) is out of pp_data bytes boundary (%hu vs "
						"%hu)\n",
						line,
						caps_idx,
						(unsigned short)((caps_idx + 1) * sizeof(hid_pp_cap)),
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->Reserved1                    = 0x%hhu%hhu%hhu\n",
					   &caps_idx,
					   &temp_uchar[0],
					   &temp_uchar[1],
					   &temp_uchar[2]) == 4) {
			...
			if (sscanf(line, "pp_data->cap[%u]->pp_cap->UnknownTokens[%u]", &caps_idx, &token_idx) == 2) {
				const size_t unknown_tokens_count =
					sizeof(pp_data->caps[0].UnknownTokens) / sizeof(pp_data->caps[0].UnknownTokens[0]);
				if (token_idx >= unknown_tokens_count) {
					fprintf(stderr,
							"Broken pp_data file, pp_data->caps[<idx>].UnknownTokens[<idx>] can have at most %zu "
							"elements, accessing %ud, (%s)",
				...
				if (sscanf(line,
						   "pp_data->cap[%u]->pp_cap->UnknownTokens[%u].Token    = 0x%02hhX\n",
						   &caps_idx,
						   &token_idx,
						   &temp_uchar[0]) == 3) {
				...
				if (sscanf(line,
						   "pp_data->cap[%u]->pp_cap->UnknownTokens[%u].Reserved = 0x%02hhX%02hhX%02hhX\n",
						   &caps_idx,
						   &token_idx,
						   &temp_uchar[0],
						   &temp_uchar[1],
				...
				if (sscanf(line,
						   "pp_data->cap[%u]->pp_cap->UnknownTokens[%u].BitField = 0x%08lX\n",
						   &caps_idx,
						   &token_idx,
						   &temp_ulong) == 3) {
			...
			// Range
			if (sscanf(line,
					   "pp_data->cap[%u]->Range.UsageMin                     = 0x%04hX\n",
					   &caps_idx,
					   &temp_usage) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->Range.UsageMax                     = 0x%04hX\n",
					   &caps_idx,
					   &temp_usage) == 2) {
			...
			// NotRange
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.Usage                        = 0x%04hX\n",
					   &caps_idx,
					   &temp_usage) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.Reserved1                    = 0x%04hX\n",
					   &caps_idx,
					   &temp_usage) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.StringIndex                  = %hu\n",
					   &caps_idx,
					   &temp_ushort) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.Reserved2                    = %hu\n",
					   &caps_idx,
					   &temp_ushort) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.DesignatorIndex              = %hu\n",
					   &caps_idx,
					   &temp_ushort) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.Reserved3                    = %hu\n",
					   &caps_idx,
					   &temp_ushort) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.DataIndex                    = %hu\n",
					   &caps_idx,
					   &temp_ushort) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotRange.Reserved4                    = %hu\n",
					   &caps_idx,
					   &temp_ushort) == 2) {
			...
			// NotButton
			if (sscanf(line,
					   "pp_data->cap[%u]->NotButton.HasNull                   = %hhu\n",
					   &caps_idx,
					   &temp_boolean[0]) == 2) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->NotButton.Reserved4                 = 0x%02hhX%02hhX%02hhX\n",
					   &caps_idx,
					   &temp_uchar[0],
					   &temp_uchar[1],
					   &temp_uchar[2]) == 4) {
			...
			if (sscanf(line,
					   "pp_data->cap[%u]->Reserved1                    = 0x%02hhu%02hhu%02hhu\n",
					   &coll_idx,
					   &temp_uchar[0],
					   &temp_uchar[1],
					   &temp_uchar[2]) == 4) {
		...
		if (sscanf(line, "pp_data->LinkCollectionArray[%u]", &coll_idx) == 1) {
			if (pp_data->FirstByteOfLinkCollectionArray == 0 || pp_data->NumberLinkCollectionNodes == 0) {
				fprintf(stderr,
						"Error reading pp_data file (%s): FirstByteOfLinkCollectionArray or NumberLinkCollectionNodes "
						"is 0 or not reported yet\n",
						line);
				continue;
			}
			if (coll_idx >= pp_data->NumberLinkCollectionNodes) {
				fprintf(stderr,
						"Error reading pp_data file (%s): the LinkCollection index (%u) is out of boundary (%hu)\n",
						line,
						coll_idx,
						pp_data->NumberLinkCollectionNodes);
				continue;
			}
			phid_pp_link_collection_node pcoll =
				(phid_pp_link_collection_node)(((unsigned char*)&pp_data->caps[0]) +
											   pp_data->FirstByteOfLinkCollectionArray);
			if (sscanf(line,
					   "pp_data->LinkCollectionArray[%u]->LinkUsage          = 0x%04hX\n",
					   &coll_idx,
					   &temp_usage) == 2) {
...
static BOOLEAN
read_hex_data_from_text_file(const char* filename, unsigned char* data_out, size_t data_size, size_t* actual_read) {
	size_t read_index = 0;
	FILE* file = NULL;
	errno_t err = fopen_s(&file, filename, "r");
	if (err != 0) {
	...
	while (fscanf(file, "%15s", buf) == 1) {
		...
		if (read_index >= data_size) {
			fprintf(stderr,
					"Buffer for file read is too small. Got only %zu bytes to read '%s'\n",
					data_size,
					filename);
			result = FALSE;
			goto end;
...
int main(int argc, char* argv[]) {
	if (argc != 3) {
		fprintf(stderr,
				"Expected 2 arguments for the test ('<>.pp_data' and '<>_expected.rpt_desc'), got: %d\n",
				argc - 1);
		return EXIT_FAILURE;
	}
	...
	if (!read_hex_data_from_text_file(argv[2],
									  expected_report_descriptor,
									  sizeof(expected_report_descriptor),
	...
	if (report_descriptor_size == expected_report_descriptor_size) {
		...
		result = EXIT_FAILURE;
		fprintf(stderr,
				"Reconstructed Report Descriptor has different size: %zu when expected %zu\n",
