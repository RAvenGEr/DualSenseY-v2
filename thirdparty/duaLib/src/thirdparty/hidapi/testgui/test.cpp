...
MainWindow::MainWindow(FXApp* app)
	...
	new FXLabel(vf, "Select a device and press Connect.", NULL, JUSTIFY_LEFT);
	new FXLabel(vf,
				...
				NULL,
				JUSTIFY_LEFT);
	new FXLabel(vf, "Data received from the device appears in the Input section.", NULL, JUSTIFY_LEFT);
	new FXLabel(vf,
	...
	// device_list = new FXList(new FXHorizontalFrame(hf,FRAME_SUNKEN|FRAME_THICK, 0,0,0,0, 0,0,0,0), NULL, 0,
	// LISTBOX_NORMAL|LAYOUT_FILL_X|LAYOUT_FILL_Y|LAYOUT_FIX_WIDTH|LAYOUT_FIX_HEIGHT, 0,0,300,200);
	device_list = new FXList(
		new FXHorizontalFrame(hf, FRAME_SUNKEN | FRAME_THICK | LAYOUT_FILL_X | LAYOUT_FILL_Y, 0, 0, 0, 0, 0, 0, 0, 0),
		NULL,
		0,
		LISTBOX_NORMAL | LAYOUT_FILL_X | LAYOUT_FILL_Y,
		0,
		0,
		300,
		200);
	...
	output_button =
		new FXButton(matrix, "Send Output Report", NULL, this, ID_SEND_OUTPUT_REPORT, BUTTON_NORMAL | LAYOUT_FILL_X);
	...
	feature_button =
		new FXButton(matrix, "Send Feature Report", NULL, this, ID_SEND_FEATURE_REPORT, BUTTON_NORMAL | LAYOUT_FILL_X);
	...
	new FXWindow(matrix);
	get_feature_button =
		new FXButton(matrix, "Get Feature Report", NULL, this, ID_GET_FEATURE_REPORT, BUTTON_NORMAL | LAYOUT_FILL_X);
...
MainWindow::~MainWindow() {
	if (connected_device) hid_close(connected_device);
	hid_exit();
	delete title_font;
}
...
void MainWindow::create() {
	...
	getApp()->addTimeout(this, ID_MAC_TIMER, 50 * timeout_scalar /*50ms*/);
}

long MainWindow::onConnect(FXObject* sender, FXSelector sel, void* ptr) {
	if (connected_device != NULL) return 1;
...
size_t MainWindow::getDataFromTextField(FXTextField* tf, char* buf, size_t len) {
	const char* delim = " ,{}\t\r\n";
